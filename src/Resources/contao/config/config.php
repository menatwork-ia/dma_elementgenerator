...
if ($GLOBALS['TL_CONFIG']['dma_eg_modules']??false)
{
...
if ($GLOBALS['TL_CONFIG']['dma_eg_content']??false)
{
...
foreach ($arrModules as $strCategory => $arrElements)
{
	foreach ($arrElements as $strElement) {
		$GLOBALS['FE_MOD'][$strCategory][DMA_EG_PREFIX.$strElement]= 'DMA\DMABundle\Contao\DMAElementGeneratorModule';
	}
}
...
{
	foreach ($arrElements as $strElement) {
		$GLOBALS['TL_CTE'][$strCategory][DMA_EG_PREFIX.$strElement]= 'DMA\DMABundle\Contao\DMAElementGeneratorContent';
	}
}

// Define dummy data widget
$GLOBALS['BE_FFL']['dma_eg_hidden'] = 'DMA\DMABundle\Contao\DMAElementGeneratorHiddenWidget';
$GLOBALS['BE_FFL']['dma_eg_combobox'] = 'DMA\DMABundle\Contao\DMAElementGeneratorComboBox';
