    ...
    protected function generateDCA($strTableName, &$dc)
    {
        ...
        while ($objElement->next())
        {
            ...
            (
                ...
                'save_callback' => array(array('DMA\\DMABundle\\Contao\\DMAElementGeneratorCallbacks','save_data'))
            );
    ...
    protected function addHyperlinkToPalette($objField)
    {
        ...
        if (is_array($arrHyperlinkData) && sizeof($arrHyperlinkData)>0)
        {
            foreach ($arrHyperlinkData as $hyperlinkData)
            {
                ...
                $GLOBALS['TL_DCA'][$this->strTable]['fields'][$title]['load_callback'] = array(array('DMA\\DMABundle\\Contao\\DMAElementGeneratorCallbacks','load_'.$objField->title . '--' . $hyperlinkData));
                $GLOBALS['TL_DCA'][$this->strTable]['fields'][$title]['save_callback'] = array(array('DMA\\DMABundle\\Contao\\DMAElementGeneratorCallbacks','save_'.$objField->title . '--' . $hyperlinkData));
    ...
    protected function addImageToPalette($objField)
    {
        ...
        if ($objField->image_data)
        {
            ...
            foreach ($arrImageData as $imageData)
            {
                ...
                $GLOBALS['TL_DCA'][$this->strTable]['fields'][$title]['load_callback'] = array(array('DMA\\DMABundle\\Contao\\DMAElementGeneratorCallbacks','load_'.$objField->title . '--' . $imageData));
                $GLOBALS['TL_DCA'][$this->strTable]['fields'][$title]['save_callback'] = array(array('DMA\\DMABundle\\Contao\\DMAElementGeneratorCallbacks','save_'.$objField->title . '--' . $imageData));
                $GLOBALS['TL_DCA'][$this->strTable]['fields'][$title]['eval']['alwaysSave'] = true;
                $GLOBALS['TL_DCA'][$this->strTable]['fields'][$title]['eval']['mandatory'] = false;
                $GLOBALS['TL_DCA'][$this->strTable]['fields'][$title]['eval']['doNotSaveEmpty'] = true;
